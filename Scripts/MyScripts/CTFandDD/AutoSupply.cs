	...
	public class AutoSupply : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( from.Skills[SkillName.Magery].Value >= 50.0 )
			{
				...
				{
					GiveItem( from, new MetalKiteShieldSC());	// Edited
				}
		...
		public static void GiveItem( Mobile m, Item item )
		{
			...
			if ( move != null )
			{		// Adicionado para não repetir leather armor
					// Modificar para potions, mas no sentido contrário
				if ( item is LeatherArms || item is LeatherChest || item is LeatherGloves || item is LeatherGorget || item is LeatherLegs || item is LeatherCap){
					item.Delete();
					return;
				}
