...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void FillBankAOS( Mobile m )
		{
			...
			// Begin third bag of artifacts
			cont = new Backpack();
			cont.Hue = 0x48F;
			cont.Name = "Bag of Artifacts Champ";
			
			PlaceItemIn( cont, 45, 66, new LieutenantOfTheBritannianRoyalGuard() );
			PlaceItemIn( cont, 69, 82, new GladiatorsCollar() );
			PlaceItemIn( cont, 93, 99, new DjinnisRing() );
			PlaceItemIn( cont, 117, 115, new DetectiveBoots() );			
			PlaceItemIn( bank, 79, 120, cont );
			// end third bag
			
			// Begin fourth bag of artifacts
			cont = new Backpack();
			cont.Hue = 0x48F;
			cont.Name = "Bag of Artifacts Halloween";
			
			PlaceItemIn( cont, 45, 66, new QuiverOfInfinity() );
			PlaceItemIn( cont, 69,82,new ConjurersGarb() );
			PlaceItemIn( cont, 93, 99,new ConjurersGrimoire() );
			PlaceItemIn( cont, 117, 115,new ConjurersTrinket() );
			PlaceItemIn( cont, 69, 132, new MaceAndShieldGlasses() );
			PlaceItemIn( cont, 93, 66, new VineCord() );
			//PlaceItemIn( cont, 117, 82, new TheTaskmaster() );
			
			
			PlaceItemIn( bank, 80, 115, cont );
			// end third bag
			
			// Begin bag of artifacts ML
			cont = new Backpack();
			cont.Hue = 0x48F;
			cont.Name = "Bag of Artifacts Halloween";
			
			PlaceItemIn( cont, 45, 66, new FeyLeggings() );
			PlaceItemIn( cont, 69,82,new CrimsonCincture() );
			PlaceItemIn( cont, 93, 99,new QuiverOfRage() );
			PlaceItemIn( cont, 117, 115,new QuiverOfElements() );
			
			
			PlaceItemIn( bank, 60, 100, cont );
			// end third bag
			...
			*/
			...
			if( TreasuresOfTokuno.Enabled )
			{
				...
				PlaceItemIn( cont, 42, 70, new Stormgrip() );
				//PlaceItemIn( cont, 42, 70, new Exiler() );
				PlaceItemIn( cont, 38, 53, new TomeOfLostKnowledge() );
				...
				PlaceItemIn( cont, 55, 83, new RuneBeetleCarapace() );
...
*/
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
			...
			CityInfo city = new CityInfo( "Cove", "WS Start Location", 2228, 1205, 0, Map.Felucca );
			...
			newChar.SendGump (new FirstGump());
